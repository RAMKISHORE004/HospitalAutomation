...
def submit(event=0):
    ...
    def openrep():
        ...
        os.startfile(path)
    def upload():
        global path,doctor_pn,emailid,pn
        ...
        bucket='5300459100532455'
        var=open(path,'rb')
        s3=boto3.resource('s3',aws_access_key_id=acces_key,
                  aws_secret_access_key=access_s_key,
                  config=Config(signature_version='s3v4')
                  )
        s3.Bucket(bucket).put_object(Key='{}.pdf'.format(pn),Body=var).Acl().put(ACL='public-read')
    def doctor_details():
        window1=Tk()
    ...
    def getdetails(event=0):
        global d_name,d_depart,p_count,doctor_pn,pn,emailid
        p_count=0
        bucket=entry.get()
        '''n=[]
        bucket=entry.get()
        try:
            ...
            for bucket in s3.buckets.all():
                n.append(bucket)
            n.index(entry.get())
            #s3.Object('530045lab', '{}.pdf'.format(entry.get())).delete()
        except:
            messagebox.showerror('error','Your are not Recommended by Doctor')
            return'''
        s3 = boto3.client('s3')
        ...
        L7.pack(side='left')
        f5.pack(fill='x')
        for i in range(x.index('tests')+1,len(x)):
            if x[i]!='':
        ...
        b1=Button(f8,text='Open Reports',command=openrep)
...
window_lab=Tk()
window_lab.configure(bg='#ff8484')
window_lab.geometry('700x650+200+2')
window_lab.resizable(0,0)
window_lab.wm_attributes('-fullscreen','true')
frame0=Frame(window_lab,bg='#ff8484')
l=Label(frame0,text="ADMIN PAGE",font=("Ariel",19),bg='#ff8484')
l.pack()
frame0.pack()
waste=Frame(window_lab,bg='#ff8484')
lab_waste=Label(waste,bg='#ff8484',).pack(fill='x')
waste.pack()
frame1=Frame(window_lab,bg='#ff8484')
username_l=Label(frame1,text='User Name : ',font=("Ariel",16),bg='#ff8484')
username_l.pack(side='left')
username_e=Entry(frame1,width=20,font=("Ariel",16))
username_e.pack(side='left')
frame1.pack()
waste1=Frame(window_lab,bg='#ff8484')
lab_waste=Label(waste1,bg='#ff8484',).pack(fill='x')
waste1.pack()
frame2=Frame(window_lab,bg='#ff8484')
password_l=Label(frame2,text='Password   : ',font=("Ariel",16),bg='#ff8484')
password_l.pack(side='left')
...
frame2.pack()
waste2=Frame(window_lab,bg='#ff8484')
...
button1.pack()
frame1.pack()
frame1_w=Frame(window_lab,bg='#ff8484')
